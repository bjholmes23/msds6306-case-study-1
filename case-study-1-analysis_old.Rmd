...
top_brewry <- head(beer_brewery,6)
bottom_brewry <- tail(beer_brewery,6)
...
#Read baseline PowerPoint file
my_pres <- read_pptx('base.pptx') 
#view(layout_summary(my_pres))
view(layout_properties(my_pres))

# States with the most brewries
my_pres <- add_slide(x=my_pres, layout = "Title Slide", master = "Office Theme") %>%
    ph_with_tabl(value = "States With the Most Breweries", location = ph_location(left = 3, top = .5, width = 8, height = 1)) %>%
