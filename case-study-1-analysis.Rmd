...
#TODO(BJH) CONVERT TO PERCENTAGE
beerData %>% 
  summarise_all(funs(sum(is.na(.))/n())) #count the number of NAs or blanks 
...
general_statistics<-grid.arrange(
    ...
    state_base + geom_polygon(data = map_data, aes(fill = breweries_per_million_person)) +
      scale_fill_viridis(option="D"),
    state_base + geom_polygon(data = map_data, aes(fill = Population)) +
      scale_fill_viridis(option="D"),
    ncol=3, nrow=2
)
...
beer_brewery %>% 
  ggplot(aes(x=Ounces, y=ABV)) + geom_point()
```
