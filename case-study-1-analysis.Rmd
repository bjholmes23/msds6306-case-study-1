...
state_base +
      geom_polygon(data = map_data, aes(fill = IBU_median)) -> IBU_median_plot

grid.arrange(
  IBU_median_plot + scale_fill_viridis(option="A") + labs(x="Virdis A", y=NULL),
  IBU_median_plot + scale_fill_viridis(option="B") + labs(x="Virdis B", y=NULL),
  IBU_median_plot + scale_fill_viridis(option="C") + labs(x="Virdis C", y=NULL),
  IBU_median_plot + scale_fill_viridis(option="D") + labs(x="Virdis D", y=NULL),
  IBU_median_plot + scale_fill_viridis(option="E") + labs(x="Virdis E", y=NULL),
  ncol=3, nrow=2
)

state_base +
      geom_polygon(data = map_data, aes(fill = ABV_median))
...
state_base +
      geom_polygon(data = map_data, aes(fill = Number_of_breweries))

state_base +
      geom_polygon(data = map_data, aes(fill = Number_of_beers))

state_base +
      geom_polygon(data = map_data, aes(fill = Number_of_styles))

state_base +
      geom_polygon(data = map_data, aes(fill = breweries_per_million_person))
...
# States with the least brewries
my_pres <- add_slide(x=my_pres, layout = "Title and Content", master = "Office Theme") %>%
...
my_pres <- add_slide(x=my_pres, layout = "Title and Content", master = "Office Theme") %>%
  ...
  ph_with_table_at(value = first_combo_brewery,
                   height = 4.5, width = 12, left = .5, top = 1,
                   last_row = FALSE, last_column = FALSE, first_row = TRUE)
...
my_pres <- add_slide(x=my_pres, layout = "Title and Content", master = "Office Theme") %>% 
  ph_with(value = "HOW MUCH MISSING DATA DID WE FIND?", location = ph_location(left = 3, top = .5, width = 8, height = 1)) %>%
  ph_with(external_img(src = 'zero.png', width = 8, height = 4.5), 
...
my_pres <- add_slide(x=my_pres, layout = "Title and Content", master = "Office Theme") %>%
  ph_with(value = "States With the Most ABV and IBU", type='Title', location = ph_location(left = 2, top = .5, width = 8, height = 1)) %>%
  ph_with(value = c("State with the most ABV beers is ",head(top_abv,1)$State), type='body', location = ph_location(left = 2, top = 2, width = 10, height = 1)) %>%
  ph_with(value = c("State with the most IBU is ",head(top_ibu,1)$State), type='body',location = ph_location(left = 2, top = 5, width = 10, height = 1)) 

#Compute the median alcohol content and international bitterness unit for each state. 
filename1 = "top_abv_summary.jpg"
filename2 = "bottom_abv_summary.jpg"
my_pres <- add_slide(x=my_pres, layout = "Title and Content", master = "Office Theme") %>% 
...
print(my_pres, target = paste(".\\anheuser-budwieser", extension, "v2.pptx", sep='') )
